...
describe('SingleProduct component', () => {
  ...
  it('renders out a div', () => {
    const wrapper = shallow(<SingleProduct product={product} />)
    expect(wrapper.find('div')).to.have.length(7)
  })

  it('renders out an h3 tag', () => {
    const wrapper = shallow(<SingleProduct product={product} />)
    expect(wrapper.find('h3').text()).to.include('Product Info:')
  })

  it('renders a product title, description, and price', () => {
    const wrapper = shallow(<SingleProduct product={product} />)
  ...
  it('renders a product total downloads, likes and tags', () => {
    const wrapper = shallow(<SingleProduct product={product} />)
